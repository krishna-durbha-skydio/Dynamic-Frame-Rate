...
std::vector<double> computeSI(const cMat& frame1, const cMat &maskValidSobel) {
	cMat gx, gy, grad;
	...
	return {meanSp.val[0], stddevSp.val[0]};
}
...
int main(int argc, char **argv) {
	...
	// write header
	if(!summary) 
		std::cerr << "frameCount,meanSI,stdSI,meanTI,stdTI" << std::endl;
	...
	while(readYUV(frame2)) {
		...
		std::vector<double> si = computeSI(uframe1, maskValidSobel);
		...
		if (!summary) {
			std::cerr << frameCount << ", " << si[0] << ", " << si[1] << ", " << meanT.val[0] << ", " << stddevT.val[0] << std::endl;
		}

		siList.push_back(si[1]);
	...
	if(!uframe1.empty()) {
		...
		if (!summary) {
			std::cerr << frameCount << "," << si[0] << ", " << si[1] << ", " << std::endl;
		}
	...
	if (summary) {
        ...
        std::cerr << "Updated Code. So, disable summary option" << std::endl;
	}
